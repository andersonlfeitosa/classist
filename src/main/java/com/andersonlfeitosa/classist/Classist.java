package com.andersonlfeitosa.classist;
...
final class Classist extends JFrame implements ListSelectionListener, DocumentListener, Runnable {
    ...
    public final boolean accept(final File pathname) {
      if (pathname.isDirectory()) {
        ...
        if (name.endsWith(".jar") || /* name.endsWith(".zip") || */name.endsWith(".ear") || name.endsWith(".war")
            || name.endsWith(".sar") || name.endsWith(".rar") || name.endsWith(".par")) {
          return true;
        }
        return false;
      }
  ...
  Classist(final String initialDirectory) throws Exception {
    ...
    final JSplitPane jsp = new JSplitPane(JSplitPane.VERTICAL_SPLIT, new JScrollPane(classList),
        new JScrollPane(jarList));
    add(jsp, gbc);
